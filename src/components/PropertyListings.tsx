...
const PropertyListings: React.FC<PropertyListingsProps> = ({ onViewDetails }) => {
  ...
  return (
    <div className="w-full bg-brand-gray min-h-screen">
      ...
      {/* 1. STICKY FILTER BAR 
          - top-16/top-24 matches the main Header height.
          - z-40 keeps it above the listings but below any popups.
      */}
      <section 
        className="sticky w-full bg-white/95 backdrop-blur-md border-b border-gray-100 py-6 z-40 shadow-sm top-16 md:top-24"
      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
            ...
            <div className="relative md:col-span-4">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-sm focus:bg-white focus:border-luxury-gold transition-all outline-none"
                onChange={(e) => { setIsLoading(true); setSearchQuery(e.target.value); setCurrentPage(1); }}
              />
            </div>

            {/* FILTERS WRAPPER */}
            <div className="flex md:grid md:grid-cols-8 md:col-span-8 gap-3 overflow-x-auto no-scrollbar pb-2 md:pb-0">
              <select 
                value={propertyType}
                className="min-w-[130px] md:col-span-2 px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-xs font-bold text-slate-700 outline-none hover:border-luxury-gold transition-all cursor-pointer shadow-sm"
              ...
              <button 
                ...
                className="min-w-[110px] md:col-span-2 flex items-center justify-center gap-2 py-3 bg-slate-50 text-slate-500 rounded-xl text-xs font-bold hover:bg-slate-100 transition-all active:scale-95 group"
              >
                <svg 
      ...
      {/* 2. MAIN CONTENT GRID 
          - pt-20 on mobile, pt-32 on desktop ensures the Header is NEVER covered by the sticky bar.
      */}
      <main className="max-w-[1440px] mx-auto px-6 pt-20 md:pt-32 pb-16">
        <div className="mb-12">
          <h2 className="text-4xl md:text-5xl font-bold text-brand-navy font-luxury tracking-tight">
            Exclusive <span className="text-luxury-gold">Listings</span>
          </h2>
          <p className="text-gray-500 text-sm mt-3 font-medium tracking-wide">
            Displaying {filteredData.length} premium properties in Nigeria
          </p>
        </div>
              ...
              <div key={item.id} className="bg-white rounded-2xl md:rounded-3xl overflow-hidden border border-gray-100 hover:shadow-2xl transition-all duration-500 group">
                <div className="relative h-64 md:h-72 overflow-hidden bg-gray-100">
                  <img 
                    ...
                    alt={item.title} 
                  />
                  <div className="absolute top-4 left-4 bg-luxury-gold text-white text-[10px] font-bold px-4 py-1.5 rounded-full uppercase tracking-widest shadow-lg">
                    {item.tag}
                  </div>
                  <div className="absolute bottom-4 left-4 bg-white/95 backdrop-blur-sm px-4 py-2 rounded-xl shadow-lg font-bold text-slate-900 text-sm md:text-base">
                    {item.price}
                  </div>
                </div>

                <div className="p-6 md:p-8">
                  <div className="flex items-center gap-1.5 text-luxury-gold text-[10px] font-bold mb-3 uppercase tracking-widest">
                    <MapPin size={12} className="md:w-3.5 md:h-3.5" /> {item.location}
                  </div>
                  <h3 className="text-xl md:text-2xl font-bold mb-3 text-brand-navy group-hover:text-luxury-gold transition-colors font-luxury line-clamp-1">
                    {item.title}
                  </h3>
                  <p className="text-gray-500 text-sm leading-relaxed line-clamp-2 mb-8">{item.desc}</p>
                  
                  <div className="flex items-center justify-between pt-6 border-t border-gray-50">
                    <div className="flex gap-4 md:gap-6 text-xs font-bold text-slate-600">
                    ...
                    <Link to={`/property/${item.id}`}>
                      <button className="text-brand-navy text-xs font-bold uppercase tracking-widest flex items-center gap-2 hover:text-luxury-gold transition-all">
        ...
        {!isLoading && totalPages > 1 && (
          <div className="mt-20 flex justify-center items-center gap-4">
            <button 
              ...
              className="p-4 border border-gray-200 rounded-full disabled:opacity-20 hover:border-luxury-gold hover:text-luxury-gold transition-all shadow-sm"
            >
              <ChevronLeft size={20} />
            </button>
